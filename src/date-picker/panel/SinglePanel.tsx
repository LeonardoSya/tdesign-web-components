...
import { DateValue, DisableDate, TdDatePickerProps } from '../type';
...
export default class DatePickerPanel extends Component<SinglePanelProps> {
  ...
  private buildTableData(props: SinglePanelProps): DatePickerTableCell[][] {
    ...
    const minBoundary = this.convertToDate(minDate, format) || new Date(-8640000000000000);
    const maxBoundary = this.convertToDate(maxDate, format) || new Date(8640000000000000);
  ...
  private getNearestYearValue(year: number, mode: SinglePanelProps['mode'] = 'date'): number {
    ...
    const { onYearChange, onMonthChange, ...rest } = this.props;
    const { partial, internalYear = [] } =
      (rest as unknown as { partial?: 'start' | 'end'; internalYear?: number[] }) || {};
  ...
  render(props: OmiProps<SinglePanelProps, any>) {
    ...
    const panelClass = classNames(`${this.classPrefix}-date-picker__panel`, props.class);
